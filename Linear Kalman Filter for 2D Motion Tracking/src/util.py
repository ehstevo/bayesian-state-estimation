...
def plot_errors(xe_t, P_t):
    ...
    for i in range(4):
        ...
        # plot error
        plt.plot(t, xe_t[i,:], 'b-', label='error')

        # plot expected 3-sigma bounds
        plt.fill_between(t, 3*expected_std[i,:], -3*expected_std[i,:], alpha=0.4)

        # plot actual 3-sigma bounds
        plt.axhline(y= 3 * truth_std[i], color='r', label='true 3-sigma')
        plt.axhline(y = -3 * truth_std[i], color='r')
